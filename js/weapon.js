...
(function () {
    ...
    try {
        class Weapon {
            ...
            update(deltaTime) {
                // 抽象化、第六感、不獸控制、心意相通、腎上腺素、虹光共鳴、暗物質是被動技能，不需要發射
                if (this.type === 'ABSTRACTION' || this.type === 'SIXTH_SENSE' || this.type === 'PON' || this.type === 'UNCONTROLLABLE_BEAST' || this.type === 'HEART_CONNECTION' || this.type === 'ADRENALINE' || this.type === 'RAINBOW_RESONANCE' || this.type === 'DARK_MATTER') {
            ...
            fire() {
                ...
                if (this.type === 'STARFALL') {
                    // 1. 確保月亮存在
                    if (!this._starfallMoon || this._starfallMoon.markedForDeletion) {
                        this._starfallMoon = new StarfallMoon(this.player);
                        Game.addProjectile(this._starfallMoon);
                    }
                    ...
                    const swordImageWidth = this.config.SWORD_IMAGE_WIDTH || 100;
                    const swordImageHeight = this.config.SWORD_IMAGE_HEIGHT || 98;
                    const fallDurationMs = this.config.FALL_DURATION_MS || 250;
        ...
        Weapon.prototype._computeFinalDamage = function (levelMul) {
            ...
            let gabrielOrbitFlat = 0;
            if (this.type === 'GABRIEL_ORBIT' && typeof TalentSystem !== 'undefined' && TalentSystem.getTalentLevel) {
                const gabrielOrbitBoostLevel = TalentSystem.getTalentLevel('gabriel_orbit_boost') || 0;
                if (gabrielOrbitBoostLevel > 0 && TalentSystem.tieredTalents && TalentSystem.tieredTalents.gabriel_orbit_boost) {
                    const effect = TalentSystem.tieredTalents.gabriel_orbit_boost.levels[gabrielOrbitBoostLevel - 1];
                    if (effect && typeof effect.flat === 'number') {
                        gabrielOrbitFlat = effect.flat;
                    }
                }
            }
