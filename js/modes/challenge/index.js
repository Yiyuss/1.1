        ...
        try {
          if (typeof window.ChallengeGifOverlay !== 'undefined') {
            ...
            if (sc && (sc.id === 'rokurost' || sc.spriteImageKey === 'player4')) {
              ...
              const player8Key = player.facingRight ? 'player8-1' : 'player8';
              const imgObj = (Game.images && Game.images[player8Key]) ? Game.images[player8Key] : null;
              let player8Src = actorSrc;
              if (imgObj && imgObj.src) {
                player8Src = imgObj.src;
              } else {
                player8Src = player.facingRight ? 'assets/images/player8-1.png' : 'assets/images/player8.png';
              }
              const baseImgObj = (Game.images && Game.images['player8']) ? Game.images['player8'] : null;
              if (baseImgObj && baseImgObj.complete) {
                const imgWidth = baseImgObj.naturalWidth || baseImgObj.width || 300;
                const imgHeight = baseImgObj.naturalHeight || baseImgObj.height || 238;
                const aspectRatio = imgWidth / imgHeight;
                const refHeight = 265;
                const renderHeight = Math.max(1, Math.floor(actorSize * (imgHeight / refHeight)));
                const renderWidth = Math.max(1, Math.floor(renderHeight * aspectRatio));
                window.ChallengeGifOverlay.showOrUpdate('challenge-player', player8Src, player.x, player.y, { width: renderWidth, height: renderHeight });
              } else {
      ...
      try {
        ...
        if (selectedMapId === 'challenge-4') {
          ...
          boss = (typeof window.ChallengeBoss4 !== 'undefined' && window.ChallengeBoss4.create)
            ? window.ChallengeBoss4.create(ctx, { maxHealth: 200000, gifUrl: (img4 ? img4.src : 'assets/images/challengeBOSS-4.gif'), gifSize: size4 })
            : null;
        } else if (selectedMapId === 'challenge-3') {
          // 使用第3張地圖的BOSS控制器（灰妲）
          ...
          boss = (typeof window.ChallengeBoss3 !== 'undefined' && window.ChallengeBoss3.create)
            ? window.ChallengeBoss3.create(ctx, { maxHealth: 160000, gifUrl: (img3 ? img3.src : 'assets/images/challengeBOSS-3.png'), gifSize: size3 })
            : null;
          ...
          boss = (typeof window.ChallengeBoss2 !== 'undefined' && window.ChallengeBoss2.create)
            ? window.ChallengeBoss2.create(ctx, { maxHealth: 130000, gifUrl: (img2 ? img2.src : 'assets/images/challengeBOSS-2.png'), gifSize: size2 })
            : null;
        } else {
