// 挑戰模式第4張地圖「LV4.黑洞」專用BOSS控制器（完全獨立，零汙染）
// 職責：維護黑洞狀態、建立/切換發射器、繪製參考光柱等視覺
(function (global) {
  ...
  function create(ctx, options) {
    ...
    const apertureImg = res && res.getImage ? res.getImage('aperture') : null;
    const apertureSrc = apertureImg ? apertureImg.src : 'assets/images/aperture.png';
    ...
    const boss = {
      ...
      gifUrl: cfg.gifUrl,
      apertureImage: apertureImg, // 保存aperture图片引用
      apertureSrc: apertureSrc, // 保存aperture图片路径
      alive: true,
      ticks: 0,
              ...
              if (next >= 0) {
                try {
                  if (global.ChallengeGifOverlay) {
                    ...
                    // 轉場改用 MP4，檔名相同：challengeBOSS-4-2.mp4
                    global.ChallengeGifOverlay.showFullscreenCover('challenge-phase-banner', 'assets/images/challengeBOSS-4-2.mp4', { opacity: 0.5, fadeInMs: 300, holdMs: 3900, fadeOutMs: 300 });
                  }
      ...
      onPhaseChanged() {
        ...
        // 黑洞地圖專屬的10種全新彈幕模式
        if (this.phase === 0) {
          // 第0階段：EventHorizon（事件視界）- 黑洞引力彈幕
          const p = global.ChallengeBossPatterns4.eventHorizon();
          ...
          // 黑洞環繞特效：5個引力光環環繞BOSS
          const bossRef = this;
          const gravityShot = function(e2, BS2){
            try {
              ...
              // 引力彈
              BS2.addBullet(e2.x, e2.y, vx, vy, 5500, 7, '#1a1a2e');
              const b = BS2.bullets[BS2.bullets.length - 1];
              if (b){
                b.shape = 'ellipse'; b.rx = 14; b.ry = 4; b.orientToVel = true; b.color2 = '#16213e'; b.restyleAt = 900;
          ...
          for (let i=0;i<haloCount;i++){
            const em2 = BS.createEmitter({ x: this.x, y: this.y, rateMs: 95, lifeMs: 0, patternFn: gravityShot });
          ...
          // 第1階段：SingularityPulse（奇點脈衝）- 脈衝式引力彈幕
          const p = global.ChallengeBossPatterns4.singularityPulse();
          const em = BS.createEmitter({ x: this.x, y: this.y, rateMs: 110, lifeMs: 0, patternFn: p });
          if (em) em.followBoss = true; this.emitters.push(em);

        } else if (this.phase === 2) {
          // 第2階段：GravitationalWaves（引力波）- 波動式彈幕
          const p = global.ChallengeBossPatterns4.gravitationalWaves();
          const em = BS.createEmitter({ x: this.x, y: this.y, rateMs: 120, lifeMs: 0, patternFn: p });
          if (em) em.followBoss = true; this.emitters.push(em);

        } else if (this.phase === 3) {
          // 第3階段：DarkMatterSwirl（暗物質旋渦）- 旋渦模式
          const p = global.ChallengeBossPatterns4.darkMatterSwirl();
          const em = BS.createEmitter({ x: this.x, y: this.y, rateMs: 105, lifeMs: 0, patternFn: p });
          if (em) em.followBoss = true; this.emitters.push(em);

        } else if (this.phase === 4) {
          // 第4階段：BlackHoleStorm（黑洞風暴）- 黑洞能量風暴
          const p = global.ChallengeBossPatterns4.blackHoleStorm();
          const em = BS.createEmitter({ x: this.x, y: this.y, rateMs: 130, lifeMs: 0, patternFn: p });
          if (em) em.followBoss = true; this.emitters.push(em);

        } else if (this.phase === 5) {
          // 第5階段：QuantumCollapse（量子坍縮）- 量子層級彈幕
          const p = global.ChallengeBossPatterns4.quantumCollapse();
          const em = BS.createEmitter({ x: this.x, y: this.y, rateMs: 95, lifeMs: 0, patternFn: p });
          if (em) em.followBoss = true; this.emitters.push(em);

        } else if (this.phase === 6) {
          // 第6階段：VoidEruption（虛空爆發）- 虛空能量彈幕
          const p = global.ChallengeBossPatterns4.voidEruption();
          const em = BS.createEmitter({ x: this.x, y: this.y, rateMs: 140, lifeMs: 0, patternFn: p });
          if (em) em.followBoss = true; this.emitters.push(em);

        } else if (this.phase === 7) {
          // 第7階段：SpacetimeRift（時空裂縫）- 時空扭曲彈幕
          const p = global.ChallengeBossPatterns4.spacetimeRift();
          const em = BS.createEmitter({ x: this.x, y: this.y, rateMs: 115, lifeMs: 0, patternFn: p });
          if (em) em.followBoss = true; this.emitters.push(em);

        } else if (this.phase === 8) {
          // 第8階段：NebulaCollapse（星雲坍縮）- 星雲坍縮彈幕
          const p = global.ChallengeBossPatterns4.nebulaCollapse();
          const em = BS.createEmitter({ x: this.x, y: this.y, rateMs: 125, lifeMs: 0, patternFn: p });
          if (em) em.followBoss = true; this.emitters.push(em);

        } else if (this.phase === 9) {
          // 第9階段：CosmicDevourer（宇宙吞噬者）- 終極黑洞彈幕
          const p = global.ChallengeBossPatterns4.cosmicDevourer();
      ...
      update(dt) {
        ...
        try {
          if (this.gifUrl && global.ChallengeGifOverlay) {
            ...
            global.ChallengeGifOverlay.showOrUpdate('challenge-boss4', this.gifUrl, this.x, this.y, sz);
          }
        } catch (_) { }
      ...
      draw(ctx2d) {
        ...
        if (!this.gifUrl) {
          ctx2d.save();
          ctx2d.fillStyle = 'rgba(26,26,46,0.6)';
          ctx2d.beginPath();
        ...
        try {
          ...
          const nowT = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
          const dtSec = Math.max(0, (nowT - t0) / 1000);
          const apertureSize = 64; // 光圈大小（直径64，半径32，与原来一样）
          const halfSize = apertureSize / 2;
          
          // 加载aperture图片（如果还没加载）
          let img = this.apertureImage;
          if (!img && this.apertureSrc) {
            img = new Image();
            img.src = this.apertureSrc;
            this.apertureImage = img;
          }
          ...
          for (let i=0;i<haloCount;i++){
            ...
            // 使用aperture.png图片代替Canvas绘制的光圈
            if (img && img.complete && img.naturalWidth > 0) {
              ctx2d.drawImage(img, hx - halfSize, hy - halfSize, apertureSize, apertureSize);
            } else {
              // 如果图片还没加载完成，使用原来的Canvas绘制作为后备
              const rg = ctx2d.createRadialGradient(hx, hy, 0, hx, hy, 32);
              rg.addColorStop(0, 'rgba(22,33,62,1)');
              rg.addColorStop(0.5, 'rgba(22,33,62,0.85)');
              rg.addColorStop(1, 'rgba(22,33,62,0)');
              ctx2d.fillStyle = rg;
  ...
  const ChallengeBoss4 = { create };
  global.ChallengeBoss4 = ChallengeBoss4;
})(this);
