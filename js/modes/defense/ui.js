// 防禦模式 UI 模組：獨立於其他模式 UI，提供 HUD 與 ESC 菜單
// 職責：
// - 顯示左上頭像/HP/EXP/能量、右上時間/金幣
// - ESC 菜單：音量控制（音樂/音效）、顯示目前天賦、暫停/繼續
// - 僅在防禦模式 enter/exit 期間掛載與清理，避免污染其他模式
// 維護守則：
// - 僅使用防禦專用 DOM ID（#defense-...），不得操作其他模式元素
...
(function(){
  ...
  const DefenseUI = {
    ...
    init(ctx, options){
      ...
      try {
        const img = ctx && ctx.resources ? ctx.resources.getImage('defense_avatar') : null;
        const avatarEl = $('#defense-avatar');
        if (avatarEl) avatarEl.src = img ? img.src : 'assets/images/player1-2.png';
      } catch(_) {}

      const music = $('#defense-music-volume');
      const sound = $('#defense-sound-volume');
      if (music) {
      ...
      this.showHUD();
      try {
        const m = document.getElementById('defense-menu');
    ...
    showHUD(){ const hud = $('#defense-ui'); if (hud) hud.style.display = ''; },
    hideHUD(){ const hud = $('#defense-ui'); if (hud) hud.style.display = 'none'; },

    openMenu(){
      const menu = $('#defense-menu'); if (menu) menu.classList.remove('hidden');
    ...
    closeMenu(){
      const menu = $('#defense-menu'); if (menu) menu.classList.add('hidden');
      try { if (typeof AudioManager !== 'undefined' && AudioManager.playSound) { AudioManager.playSound('button_click'); } } catch(_) {}
      try {
        if (this._pauseStart) {
    ...
    updateCoins(){
      try {
        ...
        const t = $('#defense-coins-text'); if (t) t.textContent = String(n);
      } catch(_) {}
    },
    ...
    _renderBars(){
      ...
      const hf = $('#defense-health-fill'); if (hf) hf.style.width = hpPct + '%';
      const xf = $('#defense-exp-fill'); if (xf) xf.style.width = expPct + '%';
      const ef = $('#defense-energy-fill'); if (ef) ef.style.width = enPct + '%';
      const ht = $('#defense-health-text'); if (ht) ht.textContent = `${Math.max(0, Math.floor(s.hp))}/${Math.max(0, Math.floor(s.hpMax))}`;
      const xt = $('#defense-exp-text'); if (xt) xt.textContent = `${Math.max(0, Math.floor(s.exp))}/${Math.max(0, Math.floor(s.expMax))}`;
      const et = $('#defense-energy-text'); if (et) et.textContent = `${Math.max(0, Math.floor(s.en))}/${Math.max(0, Math.floor(s.enMax))}`;
    },
    ...
    _startTimer(){
      const tick = () => {
        try {
          ...
          const el = $('#defense-timer'); if (el) el.textContent = fmtTime(sec);
        } catch(_) {}
      };
      tick();
    ...
    refreshTalentsList(){
      const list = $('#defense-talents-list'); if (!list) return;
      list.innerHTML = '';
      try {
        const orderedIds = [
    ...
    dispose(){
      ...
      try { const menu = $('#defense-menu'); if (menu) menu.style.display = 'none'; } catch(_) {}
