  ...
  const MainMode = {
    ...
    getManifest(){
      return {
        images: [
          ...
          // 主屋室內家具（素材已放在 js/modes/main 目錄，檔名包含原始寬高資訊）
          { key: 'f_bed_front', src: 'js/modes/main/bed-front-273x289.png' },
          { key: 'f_bed_side', src: 'js/modes/main/bed-side-431x282.png' },
          ...
          { key: 'f_desk1', src: 'js/modes/main/desk-354x212.png' },
          { key: 'f_desk2', src: 'js/modes/main/desk-372x234.png' },
          ...
          { key: 'f_small_wardrobe', src: 'js/modes/main/small wardrobe-289x213.png' },
          { key: 'f_sofa_front', src: 'js/modes/main/sofa-front-154x185.png' },
          { key: 'f_sofa_side', src: 'js/modes/main/sofa-side-145x176.png' },
          { key: 'f_storage_cabinet1', src: 'js/modes/main/storage cabinet-145x274.png' },
          ...
          { key: 'f_wardrobe2', src: 'js/modes/main/Wardrobe-244x336.png' },
          { key: 'f_wardrobe_side', src: 'js/modes/main/Wardrobe-side-158x344.png' }
        ],
        audio: [
    ...
    enter(params, ctx){
      ...
      const MapSystem = {
        ...
        /**
         * 主屋大廳家具佈局（30x20 格，TILE=48，整體約 1440x960）
         * 設計理念：參考一般 RPG / 日式宅邸的空間配置
         * - 上方：臥室區（床、衣櫃、書櫃）
         * - 中央：活動 / 交誼區（沙發、茶几、小圓桌、盆栽、小檯燈）
         * - 下方：入口與櫃台 / 接待區
         * 碰撞規則：
         * - 床、椅子、沙發、小圓桌、盆栽、小檯燈、書桌、TV櫃等：整張圖「全碰撞」
         * - 大型家具（大書櫃、大檯燈、櫃台、儲物櫃、衣櫃）：下半部碰撞，上半部無碰撞
         */
        addIndoorFurniture() {
          const tile = TILE;
          // 家具統一縮放比例（原素材已經約 50%，這裡再縮一次避免太巨大）
          // 目前為 0.325 = 原圖約 50% 再縮小 50%，若之後要微調只需修改此數值
          const FURNITURE_SCALE = 0.325;

          // 依照縮放比例取得寬高
          const getScaledSize = (img) => {
            if (!img) return { w: 0, h: 0 };
            return {
              w: Math.round(img.width * FURNITURE_SCALE),
              h: Math.round(img.height * FURNITURE_SCALE)
            };
          };
          ...
          // === 1. 下方入口與櫃台區 ===
          // 櫃台：放在入口「後面一點」，讓玩家進門有足夠緩衝空間
          // 使用 counter-833x351.png，置於畫面下方偏中，但保留約 4 格走道
          const counterImg = getImage('f_counter');
          if (counterImg) {
            const { w: cw, h: ch } = getScaledSize(counterImg);
            const cx = (this.width * tile) / 2 - cw / 2;
            // 原本是往上 2 格，現在調整為往上 4 格，避免一進門就被櫃台堵住
            const cy = (this.height * tile) - ch - 4 * tile;
            addFurniture({
              imageKey: 'f_counter',
              x: cx,
              y: cy,
              w: cw,
              h: ch,
              collision: 'halfBottom', // 櫃台下半部碰撞，上半部讓玩家在後面
              layerId: 'f-counter'
            });
          }
          ...
          // === 2. 上方臥室區（左右各一組） ===
          // 左上：床 + 衣櫃 + 書櫃
          const marginX = 2 * tile;
          const topY = 2 * tile;

          const bedFront = getImage('f_bed_front');
          if (bedFront) {
            const { w: bw, h: bh } = getScaledSize(bedFront);
            addFurniture({
              imageKey: 'f_bed_front',
              x: marginX,
              y: topY,
              w: bw,
              h: bh,
              collision: 'full', // 床：全碰撞
              layerId: 'f-bed-left'
            });
          }

          const wardrobe1 = getImage('f_wardrobe1');
          if (wardrobe1) {
            const { w: ww, h: wh } = getScaledSize(wardrobe1);
            addFurniture({
              imageKey: 'f_wardrobe1',
              x: marginX + tile * 4,
              y: topY,
              w: ww,
              h: wh,
              collision: 'halfBottom', // 衣櫃：下半部碰撞
              layerId: 'f-wardrobe-left'
            });
          }

          const bookcase = getImage('f_bookcase');
          if (bookcase) {
            const { w: bw2, h: bh2 } = getScaledSize(bookcase);
            addFurniture({
              imageKey: 'f_bookcase',
              x: marginX + tile * 8,
              y: topY,
              w: bw2,
              h: bh2,
              collision: 'halfBottom', // 書櫃：下半部碰撞
              layerId: 'f-bookcase-left'
            });
          }

          // 右上：側床 + 衣櫃 + 大書櫃
          const bedSide = getImage('f_bed_side');
          if (bedSide) {
            const { w: sw, h: sh } = getScaledSize(bedSide);
            addFurniture({
              imageKey: 'f_bed_side',
              x: this.width * tile - marginX - sw,
              y: topY,
              w: sw,
              h: sh,
              collision: 'full',
          ...
          const wardrobe2 = getImage('f_wardrobe2');
          if (wardrobe2) {
            const { w: ww2, h: wh2 } = getScaledSize(wardrobe2);
            addFurniture({
              imageKey: 'f_wardrobe2',
              x: this.width * tile - marginX - ww2 - tile * 4,
              y: topY,
              w: ww2,
              h: wh2,
              collision: 'halfBottom',
              layerId: 'f-wardrobe-right'
            });
          }

          const bigBookcase = getImage('f_big_bookcase');
          if (bigBookcase) {
            const { w: bbw, h: bbh } = getScaledSize(bigBookcase);
            addFurniture({
              imageKey: 'f_big_bookcase',
              x: this.width * tile - marginX - bbw,
              y: topY + tile * 2,
              w: bbw,
              h: bbh,
              collision: 'halfBottom',
              layerId: 'f-bigbookcase-right'
            });
          }

          // === 3. 中央交誼區 ===
          const centerX = (this.width * tile) / 2;
          const centerY = (this.height * tile) / 2;

          // 中央沙發 + 小圓桌
          const roundTableCenterImg = getImage('f_round_table2') || getImage('f_round_table1') || getImage('f_round_table3');
          const loveseatImg = getImage('f_loveseat_front'); // 雙人沙發
          if (loveseatImg && roundTableCenterImg) {
            const { w: rtw, h: rth } = getScaledSize(roundTableCenterImg);
            const { w: lsw, h: lsh } = getScaledSize(loveseatImg);

            // 小圓桌在中間
            addFurniture({
              // 中央指定使用 second 版小圓桌（若存在），否則退回目前這張的 key
              imageKey: getImage('f_round_table2') ? 'f_round_table2'
                       : (roundTableCenterImg === getImage('f_round_table1') ? 'f_round_table1' : 'f_round_table3'),
              x: centerX - rtw / 2,
              y: centerY - rth / 2,
              w: rtw,
              h: rth,
              collision: 'full', // 小圓桌：全碰撞
              layerId: 'f-round-table-center'
            });

            // 雙人沙發在圓桌下方
            addFurniture({
              imageKey: 'f_loveseat_front',
              x: centerX - lsw / 2,
              y: centerY + rth / 2,
              w: lsw,
              h: lsh,
              collision: 'full', // 雙人沙發：全碰撞
              layerId: 'f-sofa-center'
            });
          }

          // 左右兩側：椅子 + 小桌 + 盆栽 / 小檯燈
          const smallChair = getImage('f_small_chair');
          const plant = getImage('f_potted_plant');
          const lampSmall = getImage('f_lamp_small');
          const roundTable1 = getImage('f_round_table1');
          const roundTable3 = getImage('f_round_table3');

          if (smallChair && plant) {
            const { w: cw, h: ch } = getScaledSize(smallChair);
            const { w: pw, h: ph } = getScaledSize(plant);

            // 左側小角落
            addFurniture({
              imageKey: 'f_small_chair',
              x: centerX - tile * 6 - cw / 2,
              y: centerY,
              w: cw,
              h: ch,
              collision: 'full',
              layerId: 'f-smallchair-left'
            });
            addFurniture({
              imageKey: 'f_potted_plant',
              x: centerX - tile * 6 - pw / 2,
              y: centerY - ph - tile / 2,
              w: pw,
              h: ph,
              collision: 'full', // 盆栽：全碰撞
              layerId: 'f-plant-left'
            });
            // 左側使用 small round table-122x142 作為小茶几（與中央不同家具）
            if (roundTable1) {
              const { w: rt1w, h: rt1h } = getScaledSize(roundTable1);
              addFurniture({
                imageKey: 'f_round_table1',
                x: centerX - tile * 6 - rt1w / 2,
                y: centerY - rt1h / 2,
                w: rt1w,
                h: rt1h,
                collision: 'full',
                layerId: 'f-round-table-left'
              });
            }
          }

          if (smallChair && lampSmall) {
            const { w: cw2, h: ch2 } = getScaledSize(smallChair);
            const { w: lw, h: lh } = getScaledSize(lampSmall);

            // 右側小角落
            addFurniture({
              imageKey: 'f_small_chair',
              x: centerX + tile * 6 - cw2 / 2,
              y: centerY,
              w: cw2,
              h: ch2,
              collision: 'full',
              layerId: 'f-smallchair-right'
            });
            addFurniture({
              imageKey: 'f_lamp_small',
              x: centerX + tile * 6 - lw / 2,
              y: centerY - lh - tile / 2,
              w: lw,
              h: lh,
              collision: 'full', // 小檯燈：全碰撞
              layerId: 'f-lamp-right'
            });
            // 右側使用 small round table-154x155 作為小茶几
            if (roundTable3) {
              const { w: rt3w, h: rt3h } = getScaledSize(roundTable3);
              addFurniture({
                imageKey: 'f_round_table3',
                x: centerX + tile * 6 - rt3w / 2,
                y: centerY - rt3h / 2,
                w: rt3w,
                h: rt3h,
                collision: 'full',
                layerId: 'f-round-table-right'
              });
            }
          }

          // === 4. 牆邊儲物 / 書桌區 ===
          const storage1 = getImage('f_storage_cabinet1');
          const storage2 = getImage('f_storage_cabinet2');
          const desk1 = getImage('f_desk1');
          const desk2 = getImage('f_desk2');
          const bigLamp = getImage('f_big_lamp');
          const chairFront = getImage('f_chair_front');
          const chairSide = getImage('f_chair_side');
          const chairFrontSize = chairFront ? getScaledSize(chairFront) : null;
          const smallWardrobe = getImage('f_small_wardrobe');
          const sofaFrontSingle = getImage('f_sofa_front'); // 單人沙發
          const sofaSide = getImage('f_sofa_side');
          const wardrobeSide = getImage('f_wardrobe_side');

          // 左側牆邊：儲物櫃 + 書桌 + 前向椅子 + 小衣櫃
          if (storage1) {
            const { w: sw1, h: sh1 } = getScaledSize(storage1);
            addFurniture({
              imageKey: 'f_storage_cabinet1',
              x: marginX,
              y: centerY - sh1 / 2,
              w: sw1,
              h: sh1,
              collision: 'halfBottom', // 大型儲物櫃：下半部碰撞
              layerId: 'f-storage-left'
            });
          }
          if (desk1) {
            const { w: dw1, h: dh1 } = getScaledSize(desk1);
            addFurniture({
              imageKey: 'f_desk1',
              x: marginX,
              y: centerY + tile * 3,
              w: dw1,
              h: dh1,
              collision: 'full', // 書桌：全碰撞
              layerId: 'f-desk-left'
            });
            // 書桌前的椅子（面向玩家），只有在 desk1 存在時才會使用 dw1/dh1
            if (chairFront && chairFrontSize) {
              // 以預設縮放後的尺寸為基準，再稍微縮小一點，避免看起來特別高
              let cfw = Math.round(chairFrontSize.w * 0.8);
              let cfh = Math.round(chairFrontSize.h * 0.8);
              addFurniture({
                imageKey: 'f_chair_front',
                x: marginX + dw1 / 2 - cfw / 2,
                y: centerY + tile * 3 + dh1,
                w: cfw,
                h: cfh,
                collision: 'full',
                layerId: 'f-chairfront-left'
              });
            }
          }
          // 左側牆邊的小衣櫃
          if (smallWardrobe) {
            const { w: sw, h: sh } = getScaledSize(smallWardrobe);
            addFurniture({
              imageKey: 'f_small_wardrobe',
              x: marginX + tile * 2,
              y: centerY - sh - tile,
              w: sw,
              h: sh,
              collision: 'full',
              layerId: 'f-smallwardrobe-left'
            });
          }

          // 右側牆邊：儲物櫃 + TV桌 + 側向沙發 + 大立燈 + 側向衣櫃 + 第二張書桌
          if (storage2) {
            const { w: sw2, h: sh2 } = getScaledSize(storage2);
            addFurniture({
              imageKey: 'f_storage_cabinet2',
              x: this.width * tile - marginX - sw2,
              y: centerY - sh2 / 2,
              w: sw2,
              h: sh2,
              collision: 'halfBottom',
              layerId: 'f-storage-right'
            });
          }
          const tvTable = getImage('f_tv_table');
          if (tvTable) {
            const { w: tw, h: th } = getScaledSize(tvTable);
            // TV 桌
            addFurniture({
              imageKey: 'f_tv_table',
              x: this.width * tile - marginX - tw,
              y: centerY + tile * 3,
              w: tw,
              h: th,
              collision: 'full', // TV桌：全碰撞
              layerId: 'f-tv-right'
            });
            // TV 前的側向沙發（玩家面向螢幕）—— 依賴 tw/th，因此必須放在同一個區塊內
            if (sofaSide) {
              // 側向沙發僅使用全局縮放尺寸，維持和其他沙發一致比例
              const { w: ssw, h: ssh } = getScaledSize(sofaSide);
              addFurniture({
                imageKey: 'f_sofa_side',
                x: this.width * tile - marginX - tw - ssw - tile,
                y: centerY + tile * 3 + th / 2,
                w: ssw,
                h: ssh,
                collision: 'full',
                layerId: 'f-sofaside-right'
              });
            }
          }
          // 右側牆邊的大立燈
          if (bigLamp) {
            const { w: blw, h: blh } = getScaledSize(bigLamp);
            addFurniture({
              imageKey: 'f_big_lamp',
              x: this.width * tile - marginX - blw - tile,
              y: topY + tile * 4,
              w: blw,
              h: blh,
              collision: 'halfBottom', // 大型立燈：下半部碰撞
              layerId: 'f-biglamp-right'
            });
          }
          // 右側牆邊的側向衣櫃
          if (wardrobeSide) {
            const { w: wsw, h: wsh } = getScaledSize(wardrobeSide);
            addFurniture({
              imageKey: 'f_wardrobe_side',
              x: this.width * tile - marginX - wsw,
              y: centerY - wsh - tile,
              w: wsw,
              h: wsh,
              collision: 'halfBottom',
              layerId: 'f-wardrobeside-right'
            });
          }
          // 右側再放一張書桌（desk2）與側向椅子
          if (desk2) {
            const { w: dw2, h: dh2 } = getScaledSize(desk2);
            const dx2 = this.width * tile - marginX - dw2 - tile * 6;
            const dy2 = centerY + tile * 3;
            addFurniture({
              imageKey: 'f_desk2',
              x: dx2,
              y: dy2,
              ...
              h: dh2,
              collision: 'full',
              layerId: 'f-desk-right'
            });
            if (chairSide) {
              // 以原始圖片高度精確對齊：側向椅子最終高度 ≒ 前視椅子最終高度
              // 前視椅子最終高度 = (原始前視高度 * FURNITURE_SCALE * 0.8)
              let { w: baseW, h: baseH } = getScaledSize(chairSide); // = 原始側椅高度 * FURNITURE_SCALE
              let scaleK = 1;
              if (chairFront && (chairFront.naturalHeight || chairFront.height) && (chairSide.naturalHeight || chairSide.height)) {
                const rawFrontH = chairFront.naturalHeight || chairFront.height;
                const rawSideH  = chairSide.naturalHeight  || chairSide.height;
                // 目標： baseH * scaleK = rawFrontH * FURNITURE_SCALE * 0.8
                // 又 baseH = rawSideH * FURNITURE_SCALE
                // => scaleK = (rawFrontH * 0.8) / rawSideH
                scaleK = (rawFrontH * 0.8) / (rawSideH || 1);
              }
              const csw = Math.round(baseW * scaleK);
              const csh = Math.round(baseH * scaleK);
              addFurniture({
                imageKey: 'f_chair_side',
                x: dx2 + dw2 + tile / 2,
                y: dy2 + dh2 / 2 - csh / 2,
                w: csw,
                h: csh,
                collision: 'full',
                layerId: 'f-chairside-right'
              });
            }
          }

          // 額外：在中央附近放一張單人沙發，做為閱讀角落
          if (sofaFrontSingle) {
            const { w: s1w, h: s1h } = getScaledSize(sofaFrontSingle);
            addFurniture({
              imageKey: 'f_sofa_front',
              x: centerX + tile * 3,
              y: centerY - s1h - tile,
              w: s1w,
              h: s1h,
              collision: 'full',
              layerId: 'f-sofa-single'
            });
          }
        }
      };
